...
The plan lives in `plan/` at the repo root, read and updated across sessions so work can resume where it left off:

- `plan/plan.md` — the plan (template below).
- `plan/log.md` — append-only record of what's been done.
- `plan/step-{short-desc}.md` — detail for a Step complex enough to need its own sub-steps; the Step links to it.
...
3. Write `plan/plan.md` using the template below, and create an empty `plan/log.md`. Present the plan and ask if it looks right. Done when the user approves or revises it to their satisfaction. The approved Steps are the contract /implement executes.
